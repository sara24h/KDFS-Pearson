...
arch=${ARCH:-MobileNetV2}
result_dir=${RESULT_DIR:-/kaggle/working/results/run_mobilenetV2}
...
if [ "$PHASE" = "train" ]; then
    echo "torchrun --nproc_per_node=$nproc_per_node --master_port=$master_port /kaggle/working/KDFS-Peearson/main.py \
        --phase train \
        --arch $arch \
        --device cuda \
