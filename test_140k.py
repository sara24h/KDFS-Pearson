...
class Test:
    ...
    def dataload(self):
        ...
        print("==> Creating the test loader with 140k normalization...")
        image_size = (256, 256) if self.dataset_mode != 'hardfake' else (300, 300)
        
        transform_test_140k = transforms.Compose([
            transforms.Resize(image_size),
            transforms.ToTensor(),
            transforms.Normalize(
                mean=[0.5207, 0.4258, 0.3806], std=[0.2490, 0.2239, 0.2212]
            ),
        ])
        
        test_dataset_raw = dataset_manager.loader_test.dataset
        test_dataset_raw.transform = transform_test_140k
        
        self.test_loader = DataLoader(
            test_dataset_raw, batch_size=self.test_batch_size, shuffle=False,
            num_workers=self.num_workers, pin_memory=self.pin_memory
        )
        print("Test loader is ready (using 140k normalization).")
