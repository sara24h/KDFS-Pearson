...
def parse_args():
    ...
    parser.add_argument('--preserve_pruning', action='store_true', default=True,
                        help='Preserve pruning masks and load the model in pruned state (default: True)')
    parser.add_argument('--save_full_model', action='store_true',
                        help='Save the full model instead of just the state_dict')
...
preserve_pruning = args.preserve_pruning
...
# Apply identity pruning to create mask parameters if preserving pruning
if preserve_pruning:
...
cleaned_best_model_path = os.path.join(teacher_dir, 'cleaned_teacher_model_best.pth')
...
for epoch in range(epochs):
    ...
    if val_accuracy > best_val_acc:
        ...
        if save_full_model:
            # Save the full model
            torch.save(model, os.path.join(teacher_dir, 'full_teacher_model_best.pth'))
        else:
...
# Save final model
final_model_path = os.path.join(teacher_dir, 'teacher_model_final.pth')
cleaned_final_model_path = os.path.join(teacher_dir, 'cleaned_teacher_model_final.pth')
if save_full_model:
    # Save the full model
    torch.save(model, os.path.join(teacher_dir, 'full_teacher_model_final.pth'))
else:
    # Save original state_dict with pruning masks
